...
while t1 + B1 - 1 <= length(x)
    ...
    bar_x = sum(reshape(h(end:-1:1).*analysis_bfr, T1, length(h)/T1), 2); % WOLA analysis: window, time-fold to T1 samples, then sum branches
    X = fft(bar_x);   % T1-point FFT gives the subband coefficients
